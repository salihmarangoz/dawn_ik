...
struct LimitAccelerationGoal {
  ...
  bool operator()(const T* target_values, T* residuals) const // param_x, param_y, residuals
  {
    ...
    for (int target_idx=0; target_idx<robot::num_targets; target_idx++)
    {
      ...
      T qddot_max = T(shared_block.command_history[0].acceleration[target_idx]) + T(10*0.01);
      T qddot_min = T(shared_block.command_history[0].acceleration[target_idx]) - T(10*0.01);

      T qdot_max = T(shared_block.command_history[0].velocity[target_idx]) + qddot_max*T(0.01) ;
      T qdot_min = T(shared_block.command_history[0].velocity[target_idx]) + qddot_min*T(0.01);
      T q_max1 = T(shared_block.command_history[0].position[target_idx]) + qdot_max*T(0.01);
      T q_max2 = T(shared_block.command_history[0].position[target_idx]) + qdot_min*T(0.01);
