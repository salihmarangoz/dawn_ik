...
private:
  ...
  int num_joints_;
  int num_links_;
  const std::string* joint_names_;
  const int* joint_child_link_idx_;
  const int* joint_parent_link_idx_;
  const double* link_transform_translation_only_;
  const double* link_transform_quaternion_only_;
  // internal
  ros::Subscriber joint_states_sub_;
  sensor_msgs::JointStateConstPtr last_joint_state_msg_;
  bool last_joint_state_is_dirty_;
  double* global_link_transformations_; // x1,y1,z1,rw1,rx1,ry1,rz1,x2,y2,z2,rw2,rx2,ry2,rz2,... (linear1,quaternion1,linear2,quaternion2,...)
  int* joint_idx_to_msg_idx_;
