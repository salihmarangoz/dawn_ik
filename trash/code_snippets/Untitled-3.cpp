...
struct ForwardKinematicsError {
  ForwardKinematicsError(double observed_x, double observed_y)
      : observed_x(observed_x), observed_y(observed_y) {}

  template <typename T>
  bool operator()(const T* const camera, T* residuals) const // param_x, param_y, residuals
  {
    // TODO
    return true;
  }
   ...
   static ceres::CostFunction* Create(const double observed_x, const double observed_y) 
   {
     return (new ceres::AutoDiffCostFunction<ForwardKinematicsError, 3, 6>(  // num_of_residuals, size_param_x, size_param_y, ...
                 new ForwardKinematicsError(observed_x, observed_y)));
   }
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    moveit::core::RobotStatePtr current_state_ = planning_scene_monitor->getStateMonitor()->getCurrentState();
    ...
    //double final_translation[3] = {0,0,0};
    //double final_rotation[4] = {0,0,0,1};
    Eigen::Vector3d final_translation;
    final_translation << 0,0,0;
    //Eigen::Quaterniond final_rotation;

    double final_rotation[4] = {0,0,0,1};

    //final_rotation.setIdentity();

    auto prev_joint = current_state_->getJointModel(model->getRootJoint()->getName());
    ...
    while (true)
    {
      ...
      auto translation_part = t.translation();
      auto rotation_part = Eigen::Quaterniond(t.rotation());



      //double angle_axis[3] = {0, 0, 0};
      //Eigen::Matrix<double, 3, 3> matrix;
      //angle_axis[2] = *current_state_->getJointPositions(prev_joint->getName());
      //ceres::AngleAxisToRotationMatrix(angle_axis, &matrix(0, 0));

      Eigen::Vector3d Tvec(translation_part);

      double joint_angle = *current_state_->getJointPositions(prev_joint->getName());
      Eigen::Matrix3d Rmat = Eigen::AngleAxisd(joint_angle, Eigen::Vector3d::UnitZ()).toRotationMatrix();

      Eigen::Quaterniond Rmat2(Eigen::AngleAxisd(joint_angle, Eigen::Vector3d::UnitZ()));

      final_translation = final_translation + final_rotation * Tvec;
      final_rotation = final_rotation * rotation_part * Rmat;
    ...
    transformStamped2.child_frame_id = "endpoint";
    transformStamped2.transform.rotation.w = 1.0;
    transformStamped2.transform.translation.x = final_translation[0];
    transformStamped2.transform.translation.y = final_translation[1];
    transformStamped2.transform.translation.z = final_translation[2];

    Eigen::Quaterniond quad(final_rotation);

    transformStamped2.transform.rotation.x = quad.x();
    transformStamped2.transform.rotation.y = quad.y();
    transformStamped2.transform.rotation.z = quad.z();
    transformStamped2.transform.rotation.w = quad.w();
