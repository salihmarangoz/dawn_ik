...
Eigen::Vector3d endpoint(0.0,0.0,0.8);
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    if (!visual_tools_)
    {
      visual_tools_.reset(new moveit_visual_tools::MoveItVisualTools(model->getModelFrame(),"/moveit_visual_markers"));
    }
    ...
    //Eigen::Vector3d endpoint(0.0,0.0,1.0);
    ...
    moveit::core::RobotState opt_state(*current_state_);

    prev_joint = current_state_->getJointModel(model->getRootJoint()->getName());
    prev_joint = prev_joint->getChildLinkModel()->getChildJointModels()[0];
    int cur_idx=0;
    while (true)
    {
      ...
      opt_state.setJointPositions(prev_joint->getName(), &(joint_values[cur_idx]));
    ...
    opt_state.update();
    visual_tools_->publishRobotState(opt_state);
