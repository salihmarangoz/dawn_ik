...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    transformStamped2.child_frame_id = (prev_joint->getName()).c_str();
    transformStamped2.transform.rotation.w = 1.0;
    br.sendTransform(transformStamped2);

    while (true)
    {
      ROS_WARN("%s", prev_joint->getName().c_str());

      auto current_link = prev_joint->getChildLinkModel();
      //ROS_WARN("%s", current_link->getName().c_str());
      ...
      //ROS_WARN_STREAM(t.matrix());

      //auto t2 = Eigen::AngleAxisd(*current_state_->getJointPositions(prev_joint->getName()), Eigen::Vector3d::UnitZ());
      //t = t * t2;

      double angle_axis[3] = {0, 0, 0};
      Eigen::Matrix<double, 3, 3> matrix;
      angle_axis[2] = *current_state_->getJointPositions(prev_joint->getName()) + 0.01;
      ceres::AngleAxisToRotationMatrix(angle_axis, &matrix(0, 0));

      Eigen::Quaterniond quad;
      ceres::AngleAxisToQuaternion(angle_axis, &quad.coeffs()(0));

      auto t = current_link->getJointOriginTransform();

      //auto rotation = t.rotation() * t;
      //t.rotate(matrix);
      //double quad[4];
      //ceres::AngleAxisToQuaternion(angle_axis, quad);

      auto translation_part = t.translation();
      auto rotation_part = t.rotation();

      //Eigen::Quaterniond rotation_part;
      //ceres::RotationMatrixToQuaternion(t.rotation(), &rotation_part.coeffs());

      rotation_part = rotation_part * matrix;

      geometry_msgs::TransformStamped transformStamped;
      transformStamped.header.stamp = ros::Time::now();
      transformStamped.header.frame_id = (prev_joint->getParentLinkModel()->getName() ).c_str();
      transformStamped.child_frame_id = (prev_joint->getChildLinkModel()->getName()).c_str();
      ...
      Eigen::Quaterniond qqq(rotation_part);
      transformStamped.transform.rotation.x = qqq.x();
      transformStamped.transform.rotation.y = qqq.y();
      transformStamped.transform.rotation.z = qqq.z();
      transformStamped.transform.rotation.w = qqq.w();
