...
struct ReverseArray
{
    ...
    constexpr ReverseArray(const T_(&in_arr)[N_]) : arr(), size(N_)
    {
        for (int i=0; i<size; i++)
        {
            arr[size-i-1] = in_arr[i];
        }
    }
};
...
constexpr std::array<T,N> arrayInverseMapping(const T(& in_arr)[N], const T invalid_value)
{
    std::array<T,N> arr;
    return arr;
}
...
template <typename T, size_t N>
constexpr std::array<T,N> arrayReverse(const T(& in_arr)[N])
{
    std::array<T,N> arr;
    for (int i=0; i<N; i++) arr[i] = in_arr[N-i-1];
    return arr;
}
...
int main()
{
    ...
    */
    ...
    int a[] = {5,3,1,2,4};
    constexpr auto b = arrayReverse(a);
