...
constexpr CeArray<newT,newN> RemapCeArray(const CeArray<oldT,oldN>& in_arr)
{
    CeArray<newT,newN> newArr;
...
template <typename T, size_t N>
struct ReverseArray : public CeArray<T, N>
{
    // raw array initializer
    ...
    // CeArray initializer
    constexpr ReverseArray(const CeArray<T,N>& in_arr)
    {
...
int main()
{
    ...
    */
    ...
    // TEST: CeArray //////////////////////////////////////////////////////////////////
    constexpr auto con_arr = CeArray(test_arr);
    printf("size: %d\n", con_arr.size);
    for (int i=0; i<countOf(con_arr); i++){printf("%d ", con_arr[i]);}
    printf("\n");
    
    constexpr auto rf = ResizeCeArray<float,10>(CeArray(test_arr));
    printf("size: %d\n", rf.size);
    for (int i=0; i<countOf(rf); i++){printf("%.1f ", rf[i]);}
    printf("\n");
    
    /*
    constexpr auto rg = ResizeArray<int,20>(test_arr);
    printf("size: %d\n", rg.size);
    for (int i=0; i<countOf(rg); i++){printf("%d ", rg[i]);}
    printf("\n");

    constexpr auto rh = ResizeArray<float,3>(test_arr);
    printf("size: %d\n", rh.size);
    for (int i=0; i<countOf(rh); i++){printf("%.2f ", rh[i]);}
    printf("\n");
    */

    // TEST: ReverseArray //////////////////////////////////////////////////////////////////
    constexpr auto ra = ReverseArray(CeArray(test_arr));
    printf("size: %d\n", ra.size);
