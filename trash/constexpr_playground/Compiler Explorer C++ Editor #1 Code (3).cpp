...
{
    constexpr ReverseArray(const T(&in_arr)[N]): ConstexprArray<T,N>(in_arr)
    {
        
    }

    constexpr ReverseArray(const ConstexprArray<dtype,max_size>& in_arr)
    {
        for (int i=0; i<N; i++) std::array<T,N>::at(N-i-1) = in_arr[i];
    }
};
...
constexpr int a[] = {0,4,3,1,2};
constexpr auto a_arr = ConstexprArray(a);
